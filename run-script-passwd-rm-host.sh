...
local_script="/home/makwetja/change_password.sh"
...
sshpass -p "$password" ssh -o StrictHostKeyChecking=no "$username@$hostname" "sudo bash $remote_dir/change_password.sh"
...
if [ $? -eq 0 ]; then
    ...
    sshpass -p "$password" ssh -o StrictHostKeyChecking=no "$username@$hostname" "sudo rm $remote_dir/change_password.sh"
